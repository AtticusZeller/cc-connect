...
type claudeSession struct {
	...
	// pendingToolInputs stores tool info from tool_use content items so
	// we can correlate them with tool_result items via tool_use_id
	// (e.g. to skip Read results, format Edit/Write results).
	pendingToolMu     sync.Mutex
	pendingToolInputs map[string]toolInputEntry // keyed by tool_use_id
}

// toolInputEntry stores a tool name and its raw input for result formatting.
...
func (cs *claudeSession) handleAssistant(raw map[string]any) {
	...
	for _, contentItem := range contentArr {
		...
		case "tool_use":
			...
			// Store tool info for all tools so we can filter/format results.
			if toolID != "" {
				entry := toolInputEntry{toolName: toolName}
				// Keep input map for Edit/Write to format result with diff.
				if toolName == "Edit" || toolName == "Write" {
					entry.input = input
				}
				cs.pendingToolMu.Lock()
				cs.pendingToolInputs[toolID] = entry
...
func (cs *claudeSession) handleUser(raw map[string]any) {
	...
	for _, contentItem := range contentArr {
		...
		if contentType == "tool_result" {
			...
			// Look up tool name from pending tracking
			cs.pendingToolMu.Lock()
			...
			// Skip Read results — file content is too large and not useful
			if found && entry.toolName == "Read" {
				continue
			}
			...
			if isError {
				...
				// Format Edit/Write results with diff/content display
